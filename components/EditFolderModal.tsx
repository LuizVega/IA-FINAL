...
export const EditFolderModal: React.FC<EditFolderModalProps> = ({ isOpen, onClose, folderId }) => {
  ...
  useEffect(() => {
    if (isOpen && folderId) {
      ...
      if (folder) {
        setName(folder.name);
        setIsInternal(folder.isInternal || false);
        setPrefix(folder.prefix || '');
        setMargin(folder.margin || 0.30);
        setColor(folder.color || PRESET_COLORS[5].class);
      }
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-[#111] w-full max-w-lg rounded-[2rem] shadow-2xl overflow-hidden p-1 animate-in fade-in zoom-in-95 duration-200 border border-white/10">
        <div className="bg-[#161616] rounded-[1.8rem] p-6 h-full flex flex-col max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div
                  ...
                  className={`cursor-pointer rounded-2xl p-4 border transition-all duration-300 relative overflow-hidden group ${!isInternal
              ...
              <div className="flex flex-wrap gap-2 bg-black/30 p-3 rounded-2xl border border-white/5">
                {PRESET_COLORS.map((c) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border-2 transition-all relative ${c.class.split(' ')[0]} ${c.class.split(' ')[2]} ${color === c.class ? 'ring-2 ring-offset-2 ring-offset-[#1a1a1a] ring-white scale-110 opacity-100 border-transparent' : 'opacity-60 hover:opacity-100 border-transparent'
                      }`}
                    title={c.label}
                  >
            ...
            <div className="pt-4 flex justify-end gap-2">
              ...
              <Button variant="primary" onClick={handleSave} className="px-8 shadow-lg">Guardar Cambios</Button>
            </div>
          </div>
        </div>
      </div>
