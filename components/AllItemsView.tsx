...
export const AllItemsView: React.FC = () => {
  ...
  const handleClearAll = async () => {
    if (!checkAuth()) return;
    if (confirm(t('allItems.deleteWarning'))) {
  ...
  const handleContextMenuAction = (action: string) => {
    ...
    if (action === 'delete') {
      if (confirm(t('allItems.deleteItemWarning'))) {
        ...
        <div className="overflow-auto flex-1">
          <table className="min-w-full divide-y divide-white/5">
            <thead className="bg-[#050505] sticky top-0 z-10">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest">{t('allItems.colItem')}</th>
                <th className="px-6 py-4 text-center text-xs font-bold text-gray-400 uppercase tracking-widest">{t('allItems.colStock')}</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest">{t('allItems.colCategory')}</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest">{t('allItems.colSku')}</th>
                <th className="px-6 py-4 text-right text-xs font-bold text-gray-400 uppercase tracking-widest">{t('allItems.colPrice')}</th>
              </tr>
            </thead>
          ...
          {filteredItems.length === 0 && (
            <div className="flex flex-col items-center justify-center h-64 text-gray-600">
              <p className="text-lg font-medium">{t('allItems.noItems')}</p>
              <p className="text-sm">{t('allItems.noItemsDesc')}</p>
            </div>
          )}
        </div>
      </div>
