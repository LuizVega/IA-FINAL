...
export const SettingsView: React.FC = () => {
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto h-full flex flex-col lg:flex-row gap-8 overflow-hidden">

      {/* Left Column - Controls (Scrollable) */}
      <div className="flex-1 overflow-y-auto pb-32 space-y-12 pr-2 hide-scrollbar">
      ...
      {/* Right Column - Live Preview Mockup (Visible on md+) */}
      <div className="hidden lg:flex w-[380px] xl:w-[420px] flex-shrink-0 flex-col relative items-center justify-start sticky top-10 h-[calc(100vh-80px)]">

        {/* Save Bar directly above phone to associate strongly with the changes */}
        <div className="w-full flex justify-end mb-6">
          <Button
            onClick={handleSave}
            disabled={isSaving}
            className={`${saveSuccess ? 'bg-green-500 text-white' : 'bg-white text-black hover:bg-gray-200'} font-bold shadow-xl px-8 flex items-center justify-center gap-2 w-full lg:w-auto h-12 rounded-full`}
          >
            {isSaving ? (
            ...
            {isSaving ? 'Guardando...' : saveSuccess ? '¡Cambios Guardados!' : 'Guardar y Publicar'}
          </Button>
        </div>

        {/* The Preview Container */}
        <div className="w-full h-full flex-shrink-0 rounded-3xl border border-white/10 overflow-hidden relative shadow-2xl bg-black z-10">
          <div className="absolute inset-0 bg-[#050505] overflow-y-auto overflow-x-hidden no-scrollbar">
            {/* Inject the preview component here */}
            <PublicStorefront previewSettings={localSettings} />
          </div>
        </div>
      </div>
